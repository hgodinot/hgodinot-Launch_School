...
let ItemManager = {
  ...
  validInput(name, category, quantity) {
    return name.replace(/\s+/g, '').length >= 5 &&
    !category.match(/\s+/) && category.length >= 5
  },

  determineSku(name, category) {
    let first = name.replace(/\s+/g, '').substring(0, 3);
    let second = category.substring(0, 2);
    return String((first + second).toUpperCase());
  },

  addObject(name, category, quantity) {
    let sku = this.determineSku(name, category);
    this.items.push({name, category, quantity, sku});
  },

  create(name, category, quantity) {
    if (arguments.length !== 3 || !this.validInput(name, category, quantity)) {
      return { notValid: true };
    } else {
      this.addObject(name, category, quantity);
    }
  },
  ...
  delete(code) {
    this.items = this.items.filter(item => item.sku !== code);
  },
}
