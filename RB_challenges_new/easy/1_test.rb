...
class TriangleTest < Minitest::Test
  ...
  def test_nul_side_one_null
    error = assert_raises ArgumentError do
      Triangle.new(1, 0, 1)
    end

    assert_equal("All sides must be greater than 0", error.message, )
  end

  def test_nul_side_all_null
    error = assert_raises ArgumentError do
      Triangle.new(0, 0, 0)
    end
  ...
  def test_too_long_side
    ...
    assert_equal("Greater side can't be greater than the sum of the two other sides", error.message, )
  end
