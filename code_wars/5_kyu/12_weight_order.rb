def order_weight(string)
  return string if string.split == []
  
  ordered_numbers = string.split.sort do|a, b| 
    if a.to_i.digits.reverse.sum != b.to_i.digits.reverse.sum
      a.to_i.digits.reverse.sum <=> b.to_i.digits.reverse.sum
    else
      a <=> b
    end
  end
  ordered_numbers.join(" ")
end

p order_weight("    ")
p order_weight("103 123 4444 99 2000") == "2000 103 123 4444 99"
p order_weight("2000 10003 1234000 44444444 9999 11 11 22 123") == "11 11 2000 10003 22 123 1234000 44444444 9999"